...
@Component({
  selector: 'app-update-clz',
  templateUrl: './update-clz.component.html',
  styleUrls: ['./update-clz.component.scss']
})
export class UpdateClzComponent implements OnInit {
  ...
  getClz(){
    this.dataService.FindData('/get-single-clz',this.clz_id ).subscribe(res => {
      this.response =res;
      console.log(res)
  ...
  updateClz(){
    if(this.selectedFile== null && this.selectedThumbFile == null){
      ...
      fd.append('user_id',this.clzsform.value.user_id);
      ...
      this.http.post(this.apiUrl+'/update-clz',fd,{
            reportProgress:true,
            observe:'events'
      }).subscribe(event =>{
